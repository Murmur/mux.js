...
TransportParseStream = function() {
  ...
  parsePmt = function parsePmt(payload, pmt) {
    ...
    var tracksAudio = [] // array of {lang,type,pid}
    var langsAudio  = [ "fin", "swe", "eng" ]; // priority of normal_audio(hardcoded for now)
	...
	while (offset < tableEnd) {
	  ...
	  if (streamType === streamTypes.H264_STREAM_TYPE && self.programMapTable.video === null) {
		...
		tracksAudio.push( { "lang": descLang, "type": descType, "pid": pid } );
	...
	//console.log("AudioTracks", tracksAudio);
	if(tracksAudio.length>0) {
		var useAudio=null, firstAudio=null;
		for(var idxL=0; idxL<langsAudio.length && useAudio==null; idxL++) {
			for(var idx=0; idx<tracksAudio.length && useAudio==null; idx++) {
				var track = tracksAudio[idx];
				if(firstAudio==null && track.type==0x00) firstAudio=track; // any first normal_audio
				if(track.lang == langsAudio[idxL] && track.type==0x00) useAudio = track; // preferred normal_audio
			}
		}
		if(useAudio==null) useAudio = firstAudio!=null ? firstAudio : tracksAudio[0]; // failsafe to any audio
		//console.log("Audio", useAudio);
		self.programMapTable.audio = useAudio.pid;
	}
